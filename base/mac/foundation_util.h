...
//typedef unsigned long NNSSearchPathDirectory;
typedef unsigned long NSSearchPathDomainMask;
#else
//typedef unsigned int NNSSearchPathDirectory;
...
BASE_EXPORT bool GetSearchPathDirectory(unsigned long directory,
...
// If found, fills result (which must always be non-NULL) with the
// first found directory and returns true.  Otherwise, returns false.
BASE_EXPORT bool GetLocalDirectory(unsigned long directory,
                                   FilePath* result);

// Searches for directories for the given key in only the user domain.
// If found, fills result (which must always be non-NULL) with the
// first found directory and returns true.  Otherwise, returns false.
BASE_EXPORT bool GetUserDirectory(unsigned long directory,
                                  FilePath* result);
